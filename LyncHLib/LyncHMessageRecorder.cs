...
namespace LyncHLib
{
    public class LyncHMessageRecorder
    {
        ...
        void LyncHMessageRecorder_InstantMessageReceived(object sender, MessageSentEventArgs e)
        {
            ...
            {
                logger.Error(string.Format("sender is not of type InstantMessageModality, sender={0}", sender));
                return;
            }
            
            string senderName = messageModality.Endpoint.DisplayName;

            Console.WriteLine("Message from {0} received at {1}", ((InstantMessageModality)sender).Endpoint.DisplayName, DateTime.Now);
            foreach (var key in e.Contents.Keys)
            {
                var v = e.Contents[key];
